...
class NbodyGVP(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            layers.append(
                GVPMPNN(hidden_features_v, hidden_features_s, hidden_features_v, hidden_features_s, hidden_features_v, hidden_features_s)
            )
    ...
    def forward(self, batch, step, mode):
        ...
        input = torch.cat([input, batch.vel.unsqueeze(1)], dim=1)
        pred_pos = self._forward((charges, input), edge_index, batch.batch)
        pred_pos = pred_pos.reshape(batch_size, -1, 3) + mean_pos
